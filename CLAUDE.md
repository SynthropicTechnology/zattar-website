...
|---|---|---|---|---|
| H1 **manifesto** (Home) | `<Heading level="marketing-hero-display">` | 40→68px (clamp 4.44vw) | 800 | Apenas declarações curtas (≤30 chars) com presença máxima |
| H1 hero **utilitário/categoria** | `<Heading level="marketing-hero">` | 36→48px (clamp) | 800 | FAQ, /insights, /expertise, /contato, política, termos, /servicos* |
...
**Escala modular dual:** Home (`marketing-hero-display`) tem H1/H2 = 1.89× (Major Third³ — Linear/Vercel/Stripe); demais páginas (`marketing-hero`) têm H1/H2 = 1.33× (Perfect Fourth — sóbrio institucional). H2/H3 = 1.5 (Major Sixth), H3/Lead = 1.33 (PF), Lead/Body = 1.125 (Major Second). Todos ancorados em `html { font-size: 16px }` (shadcn default — **NÃO alterar root**).

**Critério de seleção Hero vs Display:** se a frase é **declaração curta de marca/manifesto** (≤30 chars, frase de impacto), use `marketing-hero-display`. Se é **rótulo de categoria/função** ou frase média/longa, use `marketing-hero`. Hero display em texto longo (≥40 chars) quebra em 2 linhas pesadas e prejudica composição.
...
## shadcn — Processo de Atualização e Extensão

**Princípio crítico:** `src/components/ui/*` é **propriedade do shadcn CLI**. Rodar `npx shadcn@latest add <name>` sobrescreve o arquivo. Customizações inline são perdidas no próximo update.

**Política em vigor:**

- **NÃO editar** `src/components/ui/*` para adicionar variantes ou sizes custom
- **Customizações vivem em** `src/components/shared/*` via composição/wrapper
- ESLint `custom/no-shadcn-ui-mutation` bloqueia reintrodução de variants drifted (`marketing-outline`, `glass-outline`)

**Padrão de wrapper (exemplo: MarketingButton):**

```tsx
// src/components/shared/marketing-button.tsx
import { Button } from "@/components/ui/button";

export function MarketingButton({ variant, className, ...props }) {
  return <Button variant="ghost" className={cn(VARIANT_CLASSES[variant], className)} {...props} />;
}
...
1. `npx shadcn@latest add <nome>` → arquivo gerado em `src/components/ui/<nome>.tsx`
2. **Não editar** o arquivo gerado
3. Se precisar customizar, criar wrapper em `src/components/shared/<nome>-zattar.tsx` (ou nome semântico)

**Atualizar componente shadcn existente:**

1. `npx shadcn@latest add <nome>` (sobrescreve com fresh)
2. Verificar se algum wrapper depende de variant/size que foi removido
3. Wrappers em `shared/*` continuam funcionando (consomem via composição)

**Tokens shadcn alinhados (não tocar):**

- **Cores:** `--background`, `--foreground`, `--primary`, `--muted`, `--card`, `--popover`, `--accent`, `--destructive`, `--border`, `--input`, `--ring`, `--chart-1..5`, `--sidebar-*` — todas presentes em `globals.css`
- **Radius:** `--radius: 0.625rem` + 7 derivações via `calc()` (template oficial)
- **Root:** `html { font-size: 16px }` — **NÃO ALTERAR**, distorce escala em rem em 12.5%

**O que shadcn NÃO opina (extensões Zattar legítimas):**

- Tipografia: tokens marketing-* + page/section/card/subsection (vide hierarquia acima)
- Layout: `--container-*`, `--section-py-*`, `--card-padding-*`, `--icon-container-*` (vide §Layout)
- Spacing: usa Tailwind nativo (4/8/12/16/...)
